 ...
 */
var go = module.exports = function (root, rootScss, cb) {
  var pkgPath = path.join(root, 'package.json');
  var pkg, rootInfo;

  if (typeof rootScss == 'function') {
    cb = rootScss;
    pkg = require(pkgPath);
    rootScss = pkg['main.scss'] && path.resolve(root, pkg['main.scss']);
    rootInfo = rootScss ? { id: idPackage(pkg), resolved: rootScss } : undefined;
  } else {
    rootInfo = rootScss ? { id: path.basename(rootScss), resolved: rootScss } : undefined;
  }
